...
def plotly_star_bar(repos):
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            range=[0, max(stars) * 1.1 if stars else 1],
            zeroline=True,
            zerolinecolor='#888',
...
def plotly_topic_bar(repos):
    ...
    fig.update_layout(
        ...
        margin=dict(l=60, r=40, t=60, b=40),
        xaxis=dict(
            range=[0, max(counts) * 1.1 if counts else 1],
            zeroline=True,
            zerolinecolor='#888',
            title_font=dict(size=18),
            tickfont=dict(size=16),
...
def generate_categorized_table(categories):
    ...
    for cat in sorted(categories.keys()):
        ...
        # Table header
        md += '| Name | Status | Stars | Creation Date | Last Updated | Primary Language | Description | Link |\n'
        md += '|---|---|---|---|---|---|---|---|\n'
        ...
        for repo in repos:
            ...
            md += f'| {name} | {private} | {stars} | {creation} | {updated} | {lang} | {desc} | [🔗]({url}) |\n'
